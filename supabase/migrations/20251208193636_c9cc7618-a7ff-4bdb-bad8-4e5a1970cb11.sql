...
-- Add the new special assistance question for all existing tours
...
SELECT 
  t.id,
  'Precisa de alguma assistência diferenciada que devemos saber?',
  'textarea',
  false,
  12,
  'standard',
...
WHERE NOT EXISTS (
  ...
  WHERE tcq.tour_id = t.id AND tcq.standard_field_key = 'assistencia_diferenciada'
);

-- Update the populate function to include the new question
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM tour_custom_questions WHERE tour_id = p_tour_id) THEN
    ...
    VALUES
      ...
      (p_tour_id, 'Precisa de alguma assistência diferenciada que devemos saber?', 'textarea', false, 12, 'standard', 'assistencia_diferenciada', true, NULL),
