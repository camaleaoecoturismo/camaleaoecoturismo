    ...
    <div className="min-h-screen bg-background">
      ...
      {partners.length > 0 && (
        <section className="py-12 px-4 border-y border-border">
          <div className="max-w-5xl mx-auto">
            <h2 className="font-figtree text-3xl md:text-4xl font-bold text-foreground mb-10 max-w-xl">
              Organizações que levaram seus times para viver a{" "}
              <span className="text-primary">reconexão com a natureza.</span>
            </h2>
            <div className="grid grid-cols-3 md:grid-cols-4 gap-x-3 gap-y-1.5 sm:gap-x-4 sm:gap-y-5 items-center">
              {partners.map((p) => (
                <a
                  key={p.id}
                  href={p.website_url || "#"}
                  ...
                  className="flex min-h-[52px] sm:min-h-[92px] items-center justify-center hover:opacity-70 transition-opacity duration-200"
                  title={p.name || ""}
                >
                  <img
                    src={p.logo_url}
                    alt={p.name || ""}
                    className="h-14 sm:h-28 md:h-32 w-auto max-w-full object-contain"
                  />
                </a>
              ))}
            </div>
