...
import {
  Users, X, MessageCircle, Send, Monitor, Smartphone, Tablet,
  MapPin, Globe, ChevronLeft, MousePointer, Clock, ArrowUpRight,
} from 'lucide-react';
...
// ─── Visitor detail + chat (inside panel) ────────────────────────────────────
...
function VisitorDetail({
  ...
  useEffect(() => {
    (supabase.from('visitor_live_chats' as any) as any)
      ...
      .limit(1)
      .then(({ data }: any) => {
        if (data?.[0]) { setChatId(data[0].id); setTab('chat'); }
      });
  }, [session.user_id_anon]);
  ...
  useEffect(() => {
    ...
    (supabase.from('visitor_live_messages' as any) as any)
      .select('*').eq('chat_id', chatId).order('sent_at', { ascending: true })
      .then(({ data }: any) => setMessages(data || []));

    const ch = supabase.channel(`widget-chat-${chatId}`)
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'visitor_live_messages',
        filter: `chat_id=eq.${chatId}`,
      }, p => setMessages(prev => [...prev, p.new as LiveMessage]))
      .subscribe();
  ...
  const startChat = async () => {
    ...
    const { data, error } = await (supabase.from('visitor_live_chats' as any) as any)
      .insert({
    ...
    if (!error && data) {
      ...
      await supabase.channel(`visitor-notify-${session.user_id_anon}`)
        .send({ type: 'broadcast', event: 'chat_started', payload: { chat_id: data.id, admin_name: adminName } });
    }
    setStarting(false);
  };
  ...
  const send = async () => {
    ...
    await (supabase.from('visitor_live_messages' as any) as any).insert({
      chat_id: chatId, visitor_anon_id: session.user_id_anon, sender_type: 'admin', content,
    });
    setSending(false);
  };
      ...
      <div className="flex border-b shrink-0">
        {(['info', 'chat'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'info' ? 'Informações' : chatId ? `💬 Chat ativo` : 'Chat'}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {tab === 'info' && (
          <div className="p-4 space-y-4">
            ...
            {/* Start chat CTA */}
            <Button
              onClick={chatId ? () => setTab('chat') : startChat}
              disabled={starting}
              size="sm"
              className="w-full"
            >
              <MessageCircle className="w-3.5 h-3.5 mr-2" />
              {chatId ? 'Abrir chat' : starting ? 'Iniciando...' : 'Iniciar conversa'}
            </Button>
          </div>
        )}
          ...
          <div className="flex flex-col h-full">
            {!chatId ? (
              <div className="flex flex-col items-center justify-center gap-3 p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">Nenhum chat ativo</p>
                <Button onClick={startChat} disabled={starting} size="sm">
              ...
              <>
                ...
                <div className="border-t px-3 py-2 flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={send} disabled={!input.trim() || sending}
                    className="w-8 h-8 rounded-full bg-primary flex items-center justify-center disabled:opacity-40 hover:opacity-90 transition-opacity shrink-0">
                    <Send className="w-3.5 h-3.5 text-primary-foreground" />
                  </button>
                </div>
                <div className="px-3 pb-2 shrink-0">
                  <button onClick={closeChat} className="text-xs text-destructive/70 hover:text-destructive transition-colors">
                    Encerrar chat
                  </button>
                </div>
              </>
