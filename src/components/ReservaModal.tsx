...
export function ReservaModal({ isOpen, onClose, tour, preSelectedQuantities }: ReservaModalProps) {
  ...
  // Card subtotal using card_price from pricing options
  const cardBaseTotal = fullParticipantsData.reduce((sum, p) => {
    const option = tour?.pricing_options?.find(o => o.id === p.pricingOptionId);
    return sum + (option?.card_price ?? p.pricingOptionPrice);
  }, 0) + selectedOptionals.reduce((sum, o) => sum + o.price * o.quantity, 0);
  const cardAfterDiscount = Math.max(0, cardBaseTotal - couponDiscount);
        ...
        <DialogContent className="max-w-2xl w-[95vw] max-h-[90vh] overflow-hidden flex flex-col p-0">
          ...
          {showCloseConfirmation && (
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 rounded-lg">
              <div className="bg-white rounded-xl shadow-xl p-6 mx-4 max-w-sm w-full text-center space-y-3">
                <p className="font-semibold text-lg">Cancelar reserva?</p>
